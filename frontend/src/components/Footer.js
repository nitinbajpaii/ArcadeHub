import React from 'react';
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import './Footer.css';

const Footer = () => {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="footer shadow-glow-up">
      <div className="footer-container">
        <div className="footer-main">
          <div className="footer-brand">
            <Link to="/" className="footer-logo">
              <span className="logo-text">🎮 ArcadeHub</span>
            </Link>
            <p className="footer-description">
              The ultimate destination for classic arcade games. Compete, win, and climb the leaderboard.
            </p>
            <div className="footer-social">
              <motion.a 
                href="https://github.com/nitinbajpaii" 
                target="_blank" 
                rel="noopener noreferrer"
                whileHover={{ y: -3, color: '#8b5cf6' }}
                aria-label="GitHub"
              >
                <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg>
              </motion.a>
              <motion.a 
                href="mailto:nitinbajpai2006@gmail.com"
                whileHover={{ y: -3, color: '#10b981' }}
                aria-label="Email"
              >
                <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
              </motion.a>
            </div>
          </div>

          <div className="footer-nav">
            <div className="footer-nav-group">
              <h4>Quick Links</h4>
              <Link to="/dashboard" className="footer-nav-link">Dashboard</Link>
              <Link to="/leaderboard" className="footer-nav-link">Leaderboard</Link>
              <Link to="/contact" className="footer-nav-link">Contact Us</Link>
            </div>
            <div className="footer-nav-group">
              <h4>Connect</h4>
              <a 
                href="https://github.com/nitinbajpaii" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="footer-nav-link"
              >
                GitHub
              </a>
              <a 
                href="mailto:nitinbajpai2006@gmail.com" 
                className="footer-nav-link"
              >
                Email Support
              </a>
            </div>
          </div>
        </div>

        <div className="footer-bottom">
          <p className="copyright">&copy; {currentYear} ArcadeHub. Built for gamers.</p>
          <div className="footer-legal">
            <span className="footer-legal-link">Privacy Policy</span>
            <span className="footer-legal-link">Terms of Service</span>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
